...
- Adds measurable metrics: Freshness, Comprehensiveness (stored, not required by your current API)
- Returns a result object + raw bundle safe to persist in JSONB
...
def _parse_date(d: Optional[str]) -> Optional[datetime]:
    # Perplexity may return ISO-ish strings. Keep tolerant.
    if not d:
        return None
...
# -----------------------------
# Package recommendation logic (no hardcoding)
# -----------------------------
...
# Lightweight “authority bonus” domains (optional)
...
def run_real_scan_perplexity(
    *,
    ...
    raw_bundle: JSON-serializable evidence payload (safe for Postgres JSONB).
    """
    ...
    # Freshness: based on citation dates if present
    ...
    # Comprehensiveness: simple key-field coverage heuristics in combined answer
    t = _norm(all_text)
    ...
    # Accuracy (proxy): official citation + consistency, penalize if official not cited + many other domains
    mismatch_penalty = 0
    if biz_domain:
    ...
    # Keep metrics in raw bundle too (handy in DB)
